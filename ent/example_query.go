...
func (eq *ExampleQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (eq *ExampleQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (eq *ExampleQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (eq *ExampleQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := eq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (eq *ExampleQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(example.Table, example.Columns, sqlgraph.NewFieldSpec(example.FieldID, field.TypeUUID))
	_spec.From = eq.sql
