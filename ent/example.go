...
type Example struct {
	...
	// ExampleString holds the value of the "ExampleString" field.
	ExampleString string `json:"ExampleString,omitempty"`
	// ExampleJSON holds the value of the "ExampleJSON" field.
	ExampleJSON  map[string]interface{} `json:"ExampleJSON,omitempty"`
	...
	for i := range columns {
		...
		case example.FieldExampleString:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field ExampleString", values[i])
			} else if value.Valid {
				e.ExampleString = value.String
			}
		case example.FieldExampleJSON:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field ExampleJSON", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &e.ExampleJSON); err != nil {
					return fmt.Errorf("unmarshal field ExampleJSON: %w", err)
				}
			}
		default:
...
func (e *Example) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("ExampleString=")
	builder.WriteString(e.ExampleString)
	builder.WriteString(", ")
	builder.WriteString("ExampleJSON=")
	builder.WriteString(fmt.Sprintf("%v", e.ExampleJSON))
	builder.WriteByte(')')
